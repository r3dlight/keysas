...
fn test_pkcs8_create_and_decrypt_der() {
    ...
    println!("Test DER private only - Private key: {:?}", &keypair.secret.to_bytes());
    println!("Test DER private only  - Public key: {:?}", &keypair.public.to_bytes());
    ...
    println!("Test DER private only  - Secret document - length: {:?}, content: {:?}", sd.len(), sd.as_bytes());

    let pk: PrivateKeyInfo = sd.decode_msg().unwrap();
    println!("Test DER private only  - Private key - content: {:?}", pk.private_key);
...
fn test_pkcs8_create_and_decrypt_with_public_der() {
    ...
    println!("Test DER with public - Private key: {:?}", &keypair.secret.to_bytes());
    println!("Test DER with public - Public key: {:?}", &keypair.public.to_bytes());
    ...
    let sd = enc_pk.decrypt(PASSWORD).unwrap();
    println!("Test DER with public - Secret document - length: {:?}, content: {:?}", sd.len(), sd.as_bytes());

    let pk: PrivateKeyInfo = sd.decode_msg().unwrap();
    println!("Test DER with public - Private key - content: {:?}", pk.private_key);
    println!("Test DER with public - Public key - content: {:?}", pk.public_key);
...
fn test_pkcs8_create_and_decrypt_with_public_pem() {
    ...
    pk_encrypted.write_pem_file(&path, "ENCRYPTED PRIVATE KEY", pkcs8::LineEnding::LF).unwrap();
    ...
    let sd = enc_pk.decrypt(PASSWORD).unwrap();
    println!("Test PEM - Secret document - length: {:?}, content: {:?}", sd.len(), sd.as_bytes());
